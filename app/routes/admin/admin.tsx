...
const Admin: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Obsługa wygasłego tokenu (błąd 401 Unauthorized)
      if (resUsers.status === 401 || resAdmins.status === 401 || resMonsters.status === 401) {
        logout();
        return;
      }
  ...
  const renderBool = (val: boolean | null | undefined) => {
    if (val === true) return <span className="text-lime-500 font-bold">✓</span>;
  ...
  return (
    ...
    >
      ...
      {/* LEWY GÓRNY RÓG - Powrót do Home oraz do User Dashboard */}
      <div className="absolute top-6 left-6 flex gap-4">
        ...
        <Link
          to="/dashboard"
          className="px-6 py-2 border border-lime-500 text-lime-500 text-sm font-mono rounded hover:bg-lime-500 hover:text-black transition-colors flex items-center"
        >
